...
void UI_PrintStringBuffer(const char *pString, uint8_t *buffer,
                          uint32_t char_width, const uint8_t *font) {
...
void UI_PrintStringSmall(const char *pString, uint8_t Start, uint8_t End,
                         uint8_t Line, uint8_t char_width,
                         const uint8_t *font) {
  const size_t Length = strlen(pString);
  const unsigned int char_spacing = char_width + 1;
...
void UI_PrintStringSmallNormal(const char *pString, uint8_t Start, uint8_t End,
                               uint8_t Line) {
...
void UI_PrintStringSmallBufferNormal(const char *pString, uint8_t *buffer) {
  UI_PrintStringBuffer(pString, buffer, ARRAY_SIZE(gFontSmall[0]),
                       (uint8_t *)gFontSmall);
}
...
void UI_DisplayFrequency(const char *string, uint8_t X, uint8_t Y,
                         bool center) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (bCanDisplay || c != ' ') {
      ...
      if (c >= '0' && c <= '9' + 1) {
        ...
        memcpy(pFb1 + 2, gFontBigDigits[c - '0'] + char_width - 3,
               char_width - 3);
      } else if (c == '.') {
        *pFb1 = 0x60;
        pFb0++;
...
void UI_DrawPixelBuffer(uint8_t (*buffer)[128], uint8_t x, uint8_t y,
                        bool black) {
...
void UI_DrawLineBuffer(uint8_t (*buffer)[128], int16_t x1, int16_t y1,
...
void UI_DrawRectangleBuffer(uint8_t (*buffer)[128], int16_t x1, int16_t y1,
                            int16_t x2, int16_t y2, bool black) {
...
void UI_DisplayClear() { memset(gFrameBuffer, 0, sizeof(gFrameBuffer)); }
