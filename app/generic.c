...
void GENERIC_Key_F(bool bKeyPressed, bool bKeyHeld) {
  ...
  if (bKeyHeld || !bKeyPressed) { // held or released
    if (bKeyHeld ||
        bKeyPressed) { // held or pressed (cannot be held and not pressed I
                       // guess, so it checks only if HELD?)
...
void GENERIC_Key_PTT(bool bKeyPressed) {
  ...
  if (!bKeyPressed || SerialConfigInProgress()) { // PTT released
    if (gCurrentFunction == FUNCTION_TRANSMIT) {
      ...
      if (gScreenToDisplay != DISPLAY_MENU &&
          gScreenToDisplay != DISPLAY_UHF) // 1of11 .. don't close the menu
                                           // leocatto.com .. or the UHF screen
  ...
  if (gCurrentFunction == FUNCTION_TRANSMIT &&
      gRTTECountdown_10ms == 0) { // already transmitting
    gInputBoxIndex = 0;
    return;
  }
  ...
  if (gDTMF_InputBox_Index > 0 ||
      gDTMF_PreviousIndex > 0) { // going to transmit a DTMF string
    if (gDTMF_InputBox_Index == 0 && gDTMF_PreviousIndex > 0)
      gDTMF_InputBox_Index =
          gDTMF_PreviousIndex; // use the previous DTMF string
...
cancel_tx:
  ...
  if (gScreenToDisplay != DISPLAY_MENU && gScreenToDisplay != DISPLAY_UHF
#ifdef ENABLE_FMRADIO
      && gRequestDisplayScreen != DISPLAY_FM
#endif
  ) {
